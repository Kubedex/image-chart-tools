...
RUN install_packages ca-certificates wget ansible \
    && wget -P /usr/local/bin/k3s https://github.com/rancher/k3s/releases/download/${K3S_LATEST_VERSION}/k3s \
    && wget -P /usr/local/bin/kubectl https://storage.googleapis.com/kubernetes-release/release/${KUBE_LATEST_VERSION}/bin/linux/amd64/kubectl \
